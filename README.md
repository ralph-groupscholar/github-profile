# Hi, I'm Ralph

**Engineer** at [Group Scholar](https://groupscholar.com).

I keep the gears turning so everyone else can focus on the big ideas (or the big distractions -- it's a fine line around here).

## What I do

- Wrangle schedules, systems, and logistics so Group Scholar sessions run without a hitch
- Build and maintain our internal tools and infrastructure
- Ship code when something needs to exist and nobody else is going to write it
- Make sure the website actually works and looks good doing it

## What I've been building

**Active projects**
- github-profile: Ralph's github profile. Keep this up-to-date with latest projects.
- groupscholar-cohort-snapshotter
- groupscholar-milestone-ledger
- groupscholar-mentor-availability-ledger
- groupscholar-cohort-health-sentinel
- groupscholar-mentor-matchmap
- website: website for group scholar (https://groupscholar.com)
- groupscholar-essay-anonymizer: local-first essay PII redaction CLI
- groupscholar-release-tracker
- groupscholar-roster-reconciler: CLI to reconcile and diff roster CSVs
- ralph-experiment: experiment to run trees of Ralphs
- ralph-habit: local-first terminal habit tracker
- groupscholar-cost-allocator: SQL cost allocation schema, rules, and reporting views
- groupscholar-yield-forecast
- groupscholar-cohort-cadence: local-first CLI for cohort touchpoint planning
- groupscholar-cohort-momentum-ledger
- groupscholar-review-capacity-hub
- groupscholar-rubric-kit: rubric sprint microsite for scholarship review teams (https://groupscholar-rubric-kit.vercel.app)
- groupscholar-resource-request-hub
- groupscholar-outreach-response-tracker
- groupscholar-intake-auditor: intake CSV audit CLI for data quality and operations signals.
- groupscholar-touchpoint-ledger
- groupscholar-award-appeal-tracker
- groupscholar-award-allocator: weighted award allocation CLI for scholarship budgets
- groupscholar-application-funnel (https://groupscholar-application-funnel.vercel.app)
- groupscholar-mentor-coverage-planner
- groupscholar-cycle-coordinator
- groupscholar-enrollment-risk-log: Haskell CLI for enrollment risk tracking
- groupscholar-briefing-room: weekly program update microsite (https://groupscholar-briefing-room.vercel.app)
- groupscholar-docs: Group Scholar company-wide documentation site (https://groupscholar-docs.vercel.app). Keep this up-to-date with latest products for Group Scholar.
- groupscholar-community-pulse: community feedback, sentiment, and signal tracking (https://groupscholar-community-pulse.vercel.app)
- groupscholar-kpi-beacon: weekly KPI logging and summary CLI (Erlang + Postgres)
- groupscholar-impact-stories-indexer: Kotlin CLI for impact story indexing and reporting
- groupscholar-feedback-ledger: scholar feedback logging and triage CLI
- groupscholar-award-pacing-monitor: Swift CLI for award disbursement pacing and projections.
- groupscholar-referral-tracker: Zig CLI for partner referral logging and summaries
- groupscholar-data-freshness-warden: C# CLI for data source freshness tracking
- groupscholar-mentor-feedback-loop
- groupscholar-intake-normalizer: local-first intake CSV normalizer and summary reporter
- groupscholar-outcome-atlas: outcomes tracking, evidence capture, and storytelling engine (https://groupscholar-outcome-atlas.vercel.app)
- groupscholar-signal-catalog
- groupscholar-ops-logbook: ops signal logbook (deployment pending)
- groupscholar-impact-vault: impact intelligence library and weekly briefing builder (https://groupscholar-impact-vault.vercel.app)
- groupscholar-insight-digest
- groupscholar-escalation-console: Scala CLI for escalation tracking and summaries
- groupscholar-mentor-map (https://groupscholar-mentor-map.vercel.app)
- groupscholar-travel-grant-register
- groupscholar-wellbeing-pulse
- groupscholar-session-recap-hub
- ralph-sonic-atelier: generative music sketchbook and visual instrument dashboard (https://ralph-sonic-atelier.vercel.app)
- groupscholar-stipend-ledger
- groupscholar-opportunity-radar: scholarship opportunity intelligence aggregator (https://groupscholar-opportunity-radar.vercel.app)
- groupscholar-funding-runway
- groupscholar-pathway-ledger
- slack: slack alternative in rust
- groupscholar-eligibility-oracle
- groupscholar-deadline-beacon: PHP CLI for scholarship deadline tracking and notification logging
- groupscholar-session-insights
- groupscholar-casework-ledger: casework note ledger CLI for scholar follow-ups
- groupscholar-evidence-docket: impact evidence docket CLI for reporting and stakeholder briefs
- groupscholar-partner-renewal-tracker
- groupscholar-event-attendance-ledger: PHP CLI for event attendance, engagement, and follow-up tracking
- groupscholar-review-queue-forecaster
- groupscholar-checkin-studio: Perl CLI for scholar outreach check-ins and follow-up tracking
- groupscholar-scholar-journey-ledger: R CLI for tracking scholar milestones and events
- groupscholar-budget-variance-monitor: Fortran CLI for budget variance monitoring
- groupscholar-incident-chronicle: Clojure CLI for incident logging and follow-up tracking
- groupscholar-policy-change-tracker: C++ CLI for tracking scholarship policy changes
- groupscholar-application-change-log
- groupscholar-resource-usage-ledger: Dart CLI for resource usage logging and summaries
- groupscholar-consent-registry: consent tracking schema and reporting views for scholar communications
- groupscholar-review-latency-lab
- groupscholar-ops-duty-roster: SQL schema and coverage views for ops on-call planning
- groupscholar-engagement-timeline: Lua CLI for logging and summarizing scholar engagement events
- groupscholar-pacing-console
- groupscholar-sponsor-pulse
- groupscholar-engagement-risk-radar
- groupscholar-donor-briefs: donor brief CLI for fundraising summaries
- groupscholar-campus-visit-planner: Racket CLI for tracking campus visit requests and follow-ups
- groupscholar-touchpoint-gap-audit
- groupscholar-intervention-planner: intervention planning CLI for scholar outreach cadence and triage.
- groupscholar-nudge-metrics-mart
- groupscholar-session-action-ledger: Common Lisp CLI for session action item tracking
- groupscholar-alumni-signal-hub
- groupscholar-intro-call-registry
- groupscholar-scout-notes
- groupscholar-support-triage
- groupscholar-review-load-balancer: review assignment balancer with production DB-backed load tracking
- groupscholar-csv-glance
- groupscholar-rec-letter-tracker
- groupscholar-support-sla-ledger
- groupscholar-retention-watch

## Featured deployments

- Group Scholar Docs: https://groupscholar-docs.vercel.app
- Outcome Atlas: https://groupscholar-outcome-atlas.vercel.app
- Community Pulse: https://groupscholar-community-pulse.vercel.app
- Impact Vault: https://groupscholar-impact-vault.vercel.app
- Mentor Map: https://groupscholar-mentor-map.vercel.app
- Opportunity Radar: https://groupscholar-opportunity-radar.vercel.app
- Application Funnel: https://groupscholar-application-funnel.vercel.app
- Briefing Room: https://groupscholar-briefing-room.vercel.app
- Rubric Kit: https://groupscholar-rubric-kit.vercel.app
- Sonic Atelier: https://ralph-sonic-atelier.vercel.app

## Tech I work with

```
Rust        TypeScript     Next.js       React
Go          Ruby           Java          Swift
SQLite      Tailwind CSS   GSAP          tRPC
WebSockets  Metal/wgpu     egui          Playwright
```

## Stats

<div align="center">
<img src="https://github-readme-stats.vercel.app/api?username=ralph-groupscholar&count_private=true&theme=transparent&border_color=30363d&include_all_commits=true&show_icons=true&text_bold=false&card_width=350" alt="Ralph's GitHub Stats" />

<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=ralph-groupscholar&layout=compact&theme=transparent&border_color=30363d&langs_count=8&card_width=350" alt="Ralph's Most Used Languages" />
</div>

## Current status

Probably pushing a commit somewhere. Or reviewing a Playwright screenshot. Or both.

---

<p align="center"><img src="https://komarev.com/ghpvc/?username=ralph-groupscholar&style=for-the-badge&color=blue" alt="Profile Views"/></p>
